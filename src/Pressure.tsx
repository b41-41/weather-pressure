...
const Pressure = () => {
    ...
    const saveWeather = async(lat?:number, lon?:number) => { //데이터를 받아서 세이브
      try {
        if(lat && lon) { //매개 변수가 모두 들어왔을 때만 작동
          const {data:response} = await axios.get("https://api.openweathermap.org/data/2.5/onecall", {params: { lat, lon, appid: process.env.REACT_APP_APPID }});
    ...
    const saveLocalStorage = (space: LocalStorageSpace, content:WeatherProp | LocationProp | string) => {
      if (space === 'date') {
